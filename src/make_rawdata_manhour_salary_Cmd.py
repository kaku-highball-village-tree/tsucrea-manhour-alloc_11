...
NEW_RAWDATA_STEP0027_STATUTORY_WELFARE_FILE_PATTERN: re.Pattern[str] = re.compile(r"^新_ローデータ_シート_step0027_法定福利費_\d{4}年\d{2}月\.tsv$")
NEW_RAWDATA_STEP0028_STATUTORY_WELFARE_FILE_PATTERN: re.Pattern[str] = re.compile(r"^新_ローデータ_シート_step0028_法定福利費_\d{4}年\d{2}月\.tsv$")
NEW_RAWDATA_STEP0029_STATUTORY_WELFARE_FILE_PATTERN: re.Pattern[str] = re.compile(r"^新_ローデータ_シート_step0029_法定福利費_\d{4}年\d{2}月\.tsv$")
NEW_RAWDATA_STEP0030_STATUTORY_WELFARE_FILE_PATTERN: re.Pattern[str] = re.compile(r"^新_ローデータ_シート_step0030_法定福利費_\d{4}年\d{2}月\.tsv$")
...
def build_make_rawdata_step_result_log_path(objInputPath: Path) -> Path:
    return objInputPath.resolve().parent / "log_make_rawdata_step_result.tsv"


def _normalize_log_cell_text(pszValue: str) -> str:
    return (pszValue or "").replace("\t", " ").replace("\r", " ").replace("\n", " ").strip()


def append_make_rawdata_step_result_log(
    objInputPath: Path,
    pszRunId: str,
    pszStepName: str,
    pszResult: str,
    objOutputPath: Path | None = None,
    objError: Exception | None = None,
    objErrorTxtPath: Path | None = None,
) -> None:
    objLogPath: Path = build_make_rawdata_step_result_log_path(objInputPath)
    pszTimestamp: str = datetime.now().isoformat(timespec="seconds")
    pszErrorType: str = "" if objError is None else type(objError).__name__
    pszErrorMessage: str = "" if objError is None else _normalize_log_cell_text(str(objError))
    objLogLine: str = "\t".join([
        pszTimestamp,
        pszRunId,
        _normalize_log_cell_text(pszStepName),
        _normalize_log_cell_text(str(objInputPath)),
        _normalize_log_cell_text("" if objOutputPath is None else str(objOutputPath)),
        _normalize_log_cell_text(pszResult),
        _normalize_log_cell_text(pszErrorType),
        _normalize_log_cell_text(pszErrorMessage),
        _normalize_log_cell_text("" if objErrorTxtPath is None else str(objErrorTxtPath)),
    ])
    try:
        with objLogPath.open("a", encoding="utf-8", newline="") as objFile:
...
def process_new_rawdata_step0028_from_step0027(
    ...
    objOutputRows[1][iInsertIndex + 1] = "6110"
    objOutputRows[1][iInsertIndex + 2] = "0"
    objOutputRows[1][iInsertIndex + 9] = "社員給与プロジェクト振替"
...
def load_org_table_account_codes_for_statutory_welfare(
    objOrgTableTsvPath: Path,
) -> tuple[Dict[str, List[str]], List[str]]:
    if not objOrgTableTsvPath.exists():
        raise FileNotFoundError(f"管轄PJ表_法定福利.tsv が見つかりません。Path = {objOrgTableTsvPath}")
    ...
    objHeaderRow: List[str] = [("" if objCell is None else str(objCell)).strip() for objCell in objRows[0]]
    if "PJコード" not in objHeaderRow or "勘定科目コード" not in objHeaderRow:
        objErrors.append("管轄PJ表_法定福利.tsv に必須列（PJコード / 勘定科目コード）がありません。")
        return {}, objErrors
    iProjectCodeIndex: int = objHeaderRow.index("PJコード")
    iAccountCodeIndex: int = objHeaderRow.index("勘定科目コード")
...
def process_new_rawdata_step0023_statutory_welfare_from_step0022_statutory_welfare(
    ...
    write_sheet_to_tsv(objOutputPath, objOutputRows)
    process_new_rawdata_step0024_statutory_welfare_from_step0023_statutory_welfare_with_org_table(objOutputPath)
    return 0


def build_new_rawdata_step0024_statutory_welfare_output_path_from_step0023_statutory_welfare(
    objStep0023StatutoryWelfarePath: Path,
) -> Path:
    pszFileName: str = objStep0023StatutoryWelfarePath.name
    if "_step0023_法定福利費_" not in pszFileName:
        raise ValueError(f"Input is not step0023 statutory welfare file: {objStep0023StatutoryWelfarePath}")
    pszOutputFileName: str = pszFileName.replace("_step0023_法定福利費_", "_step0024_法定福利費_", 1)
    return objStep0023StatutoryWelfarePath.resolve().parent / pszOutputFileName


def build_new_rawdata_step0024_statutory_welfare_error_path_from_step0024_statutory_welfare(
    objStep0024StatutoryWelfarePath: Path,
) -> Path:
    return objStep0024StatutoryWelfarePath.resolve().parent / f"{objStep0024StatutoryWelfarePath.stem}_error.txt"


def process_new_rawdata_step0024_statutory_welfare_from_step0023_statutory_welfare_with_org_table(
    objStep0023StatutoryWelfarePath: Path,
) -> int:
    objInputRows: List[List[str]] = read_tsv_rows(objStep0023StatutoryWelfarePath)
    if not objInputRows:
        raise ValueError(f"Input TSV has no rows: {objStep0023StatutoryWelfarePath}")

    objOrgTablePath: Path = objStep0023StatutoryWelfarePath.resolve().parent / "管轄PJ表_法定福利.tsv"
    objCodeToSubaccounts, objErrorLines = load_org_table_subaccount_codes_for_salary(objOrgTablePath)
    ...
    for iRowIndex, objRow in enumerate(objInputRows):
        ...
        if iRowIndex == 0:
            ...
            pszSubaccountCode = ""
            if iProjectNameIndex == -1:
                objErrorLines.append("step0023_法定福利費 にプロジェクト名列がありません。")
            else:
                pszProjectName: str = objNewRow[iProjectNameIndex] if len(objNewRow) > iProjectNameIndex else ""
                pszProjectCodePrefix: str = extract_project_code_prefix_step0017(pszProjectName)
                objCandidates: List[str] = objCodeToSubaccounts.get(pszProjectCodePrefix, [])
                if len(objCandidates) == 0:
                    objErrorLines.append(
                        f"管轄PJ表_法定福利.tsv に該当する補助科目コードがありません。プロジェクト名={pszProjectName} コード={pszProjectCodePrefix}"
                    )
                elif len(objCandidates) >= 2:
                    pszSubaccountCode = objCandidates[0]
                    objErrorLines.append(
    ...
    objOutputPath: Path = build_new_rawdata_step0024_statutory_welfare_output_path_from_step0023_statutory_welfare(
        objStep0023StatutoryWelfarePath
    )
    write_sheet_to_tsv(objOutputPath, objOutputRows)
    if objErrorLines:
        objErrorPath: Path = build_new_rawdata_step0024_statutory_welfare_error_path_from_step0024_statutory_welfare(
            objOutputPath
        )
        objErrorPath.write_text("\n".join(objErrorLines) + "\n", encoding="utf-8")
    process_new_rawdata_step0025_statutory_welfare_from_step0024_statutory_welfare_with_org_table(objOutputPath)
    return 0


def build_new_rawdata_step0025_statutory_welfare_output_path_from_step0024_statutory_welfare(
    objStep0024StatutoryWelfarePath: Path,
) -> Path:
    pszFileName: str = objStep0024StatutoryWelfarePath.name
    if "_step0024_法定福利費_" not in pszFileName:
        raise ValueError(f"Input is not step0024 statutory welfare file: {objStep0024StatutoryWelfarePath}")
    pszOutputFileName: str = pszFileName.replace("_step0024_法定福利費_", "_step0025_法定福利費_", 1)
    return objStep0024StatutoryWelfarePath.resolve().parent / pszOutputFileName


def build_new_rawdata_step0025_statutory_welfare_error_path_from_step0025_statutory_welfare(
    objStep0025StatutoryWelfarePath: Path,
) -> Path:
    return objStep0025StatutoryWelfarePath.resolve().parent / f"{objStep0025StatutoryWelfarePath.stem}_error.txt"


def process_new_rawdata_step0025_statutory_welfare_from_step0024_statutory_welfare_with_org_table(
    objStep0024StatutoryWelfarePath: Path,
) -> int:
    objInputRows: List[List[str]] = read_tsv_rows(objStep0024StatutoryWelfarePath)
    if not objInputRows:
    ...
    for iRowIndex, objRow in enumerate(objInputRows):
        ...
        if iRowIndex == 0:
            ...
            pszAccountCode = ""
            if iProjectNameIndex == -1:
                objErrorLines.append("step0024_法定福利費 にプロジェクト名列がありません。")
            else:
                pszProjectName: str = objNewRow[iProjectNameIndex] if len(objNewRow) > iProjectNameIndex else ""
                pszProjectCodePrefix: str = extract_project_code_prefix_step0017(pszProjectName)
                objCandidates: List[str] = objCodeToAccounts.get(pszProjectCodePrefix, [])
                if len(objCandidates) == 0:
                    objErrorLines.append(
                        f"管轄PJ表_法定福利.tsv に該当する勘定科目コードがありません。プロジェクト名={pszProjectName} コード={pszProjectCodePrefix}"
                    )
                elif len(objCandidates) >= 2:
                    pszAccountCode = objCandidates[0]
                    objErrorLines.append(
    ...
    objOutputPath: Path = build_new_rawdata_step0025_statutory_welfare_output_path_from_step0024_statutory_welfare(
        objStep0024StatutoryWelfarePath
    )
    write_sheet_to_tsv(objOutputPath, objOutputRows)
    if objErrorLines:
        objErrorPath: Path = build_new_rawdata_step0025_statutory_welfare_error_path_from_step0025_statutory_welfare(
            objOutputPath
        )
        objErrorPath.write_text("\n".join(objErrorLines) + "\n", encoding="utf-8")
...
def build_new_rawdata_step0027_statutory_welfare_output_path_from_step0026_statutory_welfare(
    objStep0026StatutoryWelfarePath: Path,
) -> Path:
    pszFileName: str = objStep0026StatutoryWelfarePath.name
    if "_step0026_法定福利費_" not in pszFileName:
        raise ValueError(f"Input is not step0026 statutory welfare file: {objStep0026StatutoryWelfarePath}")
    pszOutputFileName: str = pszFileName.replace("_step0026_法定福利費_", "_step0027_法定福利費_", 1)
    return objStep0026StatutoryWelfarePath.resolve().parent / pszOutputFileName


def build_statutory_welfare_voucher_date_from_step0026_path(
    objStep0026StatutoryWelfarePath: Path,
) -> str:
    objMatch: re.Match[str] | None = re.match(
        r"^新_ローデータ_シート_step0026_法定福利費_(\d{4})年(\d{2})月\.tsv$",
        objStep0026StatutoryWelfarePath.name,
    )
    if objMatch is None:
        raise ValueError(f"Input is not step0026 statutory welfare file: {objStep0026StatutoryWelfarePath}")
    iYear: int = int(objMatch.group(1))
...
def process_new_rawdata_step0026_statutory_welfare_from_step0025_statutory_welfare_with_org_table(
    ...
    if objErrorLines:
        ...
        objErrorPath.write_text("\n".join(objErrorLines) + "\n", encoding="utf-8")
    process_new_rawdata_step0027_statutory_welfare_from_step0026_statutory_welfare(objOutputPath)
    return 0


def process_new_rawdata_step0027_statutory_welfare_from_step0026_statutory_welfare(
    objStep0026StatutoryWelfarePath: Path,
) -> int:
    objInputRows: List[List[str]] = read_tsv_rows(objStep0026StatutoryWelfarePath)
    if not objInputRows:
        raise ValueError(f"Input TSV has no rows: {objStep0026StatutoryWelfarePath}")

    pszVoucherDate: str = build_statutory_welfare_voucher_date_from_step0026_path(objStep0026StatutoryWelfarePath)
    objHeaderRow: List[str] = list(objInputRows[0])
    iDebitDepartmentCodeIndex: int = (
        objHeaderRow.index("借方部門コード")
        if "借方部門コード" in objHeaderRow
    ...
    objOutputPath: Path = build_new_rawdata_step0027_statutory_welfare_output_path_from_step0026_statutory_welfare(
        objStep0026StatutoryWelfarePath
    )
    write_sheet_to_tsv(objOutputPath, objOutputRows)
    process_new_rawdata_step0019_statutory_welfare_from_step0018_statutory_welfare(objOutputPath)
    return 0
...
def build_new_rawdata_step0027_statutory_welfare_error_path_from_step0027_statutory_welfare(
    objStep0027StatutoryWelfarePath: Path,
) -> Path:
    return objStep0027StatutoryWelfarePath.resolve().parent / f"{objStep0027StatutoryWelfarePath.stem}_error.txt"


def build_new_rawdata_step0028_statutory_welfare_output_path_from_step0027_statutory_welfare(
    objStep0027StatutoryWelfarePath: Path,
) -> Path:
    pszFileName: str = objStep0027StatutoryWelfarePath.name
    if "_step0027_法定福利費_" not in pszFileName:
        raise ValueError(f"Input is not step0027 statutory welfare file: {objStep0027StatutoryWelfarePath}")
    pszOutputFileName: str = pszFileName.replace("_step0027_法定福利費_", "_step0028_法定福利費_", 1)
    return objStep0027StatutoryWelfarePath.resolve().parent / pszOutputFileName


def build_new_rawdata_step0028_statutory_welfare_error_path_from_step0028_statutory_welfare(
    objStep0028StatutoryWelfarePath: Path,
) -> Path:
    return objStep0028StatutoryWelfarePath.resolve().parent / f"{objStep0028StatutoryWelfarePath.stem}_error.txt"


def build_new_rawdata_step0029_statutory_welfare_output_path_from_step0028_statutory_welfare(
    objStep0028StatutoryWelfarePath: Path,
) -> Path:
    pszFileName: str = objStep0028StatutoryWelfarePath.name
    if "_step0028_法定福利費_" not in pszFileName:
        raise ValueError(f"Input is not step0028 statutory welfare file: {objStep0028StatutoryWelfarePath}")
    pszOutputFileName: str = pszFileName.replace("_step0028_法定福利費_", "_step0029_法定福利費_", 1)
    return objStep0028StatutoryWelfarePath.resolve().parent / pszOutputFileName


def build_new_rawdata_step0030_statutory_welfare_output_path_from_step0029_statutory_welfare(
    objStep0029StatutoryWelfarePath: Path,
) -> Path:
    pszFileName: str = objStep0029StatutoryWelfarePath.name
    if "_step0029_法定福利費_" not in pszFileName:
        raise ValueError(f"Input is not step0029 statutory welfare file: {objStep0029StatutoryWelfarePath}")
    pszOutputFileName: str = pszFileName.replace("_step0029_法定福利費_", "_step0030_法定福利費_", 1)
    return objStep0029StatutoryWelfarePath.resolve().parent / pszOutputFileName


def build_new_rawdata_step0030_statutory_welfare_error_path_from_step0030_statutory_welfare(
    objStep0030StatutoryWelfarePath: Path,
) -> Path:
    return objStep0030StatutoryWelfarePath.resolve().parent / f"{objStep0030StatutoryWelfarePath.stem}_error.txt"
...
def process_new_rawdata_step0027_statutory_welfare_from_step0026_statutory_welfare(
    ...
    try:
        ...
        objErrorLines: List[str] = [
            str(objException),
            "期待形式: 新_ローデータ_シート_step0026_法定福利費_YYYY年MM月.tsv",
        ]
        objErrorPath: Path = build_new_rawdata_step0027_statutory_welfare_error_path_from_step0027_statutory_welfare(
            objOutputPath
        )
        objErrorPath.write_text("\n".join(objErrorLines) + "\n", encoding="utf-8")
        raise
...
def process_new_rawdata_step0027_statutory_welfare_from_step0026_statutory_welfare(
    ...
    write_sheet_to_tsv(objOutputPath, objOutputRows)
    process_new_rawdata_step0028_statutory_welfare_from_step0027_statutory_welfare(objOutputPath)
    return 0


def process_new_rawdata_step0028_statutory_welfare_from_step0027_statutory_welfare(
    objStep0027StatutoryWelfarePath: Path,
) -> int:
    objInputRows: List[List[str]] = read_tsv_rows(objStep0027StatutoryWelfarePath)
    if not objInputRows:
        raise ValueError(f"Input TSV has no rows: {objStep0027StatutoryWelfarePath}")

    objHeaderRow: List[str] = list(objInputRows[0])
    objErrorLines: List[str] = []
    objOutputPath: Path = build_new_rawdata_step0028_statutory_welfare_output_path_from_step0027_statutory_welfare(
    ...
    if objDeleteColumnName in objHeaderRow:
        ...
        objErrorLines.append(f"step0027_法定福利費 に{objDeleteColumnName}列がありません。")
    ...
    if objErrorLines:
        ...
        objErrorPath.write_text("\n".join(objErrorLines) + "\n", encoding="utf-8")
    process_new_rawdata_step0029_statutory_welfare_from_step0028_statutory_welfare(objOutputPath)
    return 0


def process_new_rawdata_step0029_statutory_welfare_from_step0028_statutory_welfare(
    objStep0028StatutoryWelfarePath: Path,
) -> int:
    objInputRows: List[List[str]] = read_tsv_rows(objStep0028StatutoryWelfarePath)
    if not objInputRows:
        raise ValueError(f"Input TSV has no rows: {objStep0028StatutoryWelfarePath}")
    ...
    objOutputRows[1][iInsertIndex + 2] = "0"
    objOutputRows[1][iInsertIndex + 9] = "社員　法定福利プロジェクト振替\n"

    objOutputPath: Path = build_new_rawdata_step0029_statutory_welfare_output_path_from_step0028_statutory_welfare(
        objStep0028StatutoryWelfarePath
    )
    write_sheet_to_tsv(objOutputPath, objOutputRows)
    process_new_rawdata_step0030_statutory_welfare_from_step0029_statutory_welfare(objOutputPath)
    return 0


def process_new_rawdata_step0030_statutory_welfare_from_step0029_statutory_welfare(
    objStep0029StatutoryWelfarePath: Path,
) -> int:
    objInputRows: List[List[str]] = read_tsv_rows(objStep0029StatutoryWelfarePath)
    if not objInputRows:
        raise ValueError(f"Input TSV has no rows: {objStep0029StatutoryWelfarePath}")

    objOutputRows: List[List[str]] = [list(objRow) for objRow in objInputRows]
    objHeaderRow: List[str] = list(objOutputRows[0])
    objErrorLines: List[str] = []
    objOutputPath: Path = build_new_rawdata_step0030_statutory_welfare_output_path_from_step0029_statutory_welfare(
        objStep0029StatutoryWelfarePath
    )

    if "借方本体金額" not in objHeaderRow or "貸方本体金額" not in objHeaderRow:
        if "借方本体金額" not in objHeaderRow:
            objErrorLines.append("step0029_法定福利費 に借方本体金額列がありません。")
        if "貸方本体金額" not in objHeaderRow:
            objErrorLines.append("step0029_法定福利費 に貸方本体金額列がありません。")
        objErrorPath: Path = build_new_rawdata_step0030_statutory_welfare_error_path_from_step0030_statutory_welfare(
            objOutputPath
        )
        objErrorPath.write_text("\n".join(objErrorLines) + "\n", encoding="utf-8")
        raise ValueError(f"Missing required headers. Input = {objStep0029StatutoryWelfarePath}")
...
def main() -> int:
    ...
    pszRunId: str = datetime.now().strftime("%Y%m%dT%H%M%S.%f")
    ...
    objNewRawdataStep0026StatutoryWelfarePaths: List[Path] = []
    objNewRawdataStep0027StatutoryWelfarePaths: List[Path] = []
    objNewRawdataStep0028StatutoryWelfarePaths: List[Path] = []
    objNewRawdataStep0029StatutoryWelfarePaths: List[Path] = []
    ...
    for pszInputXlsxPath in objArgs.pszInputXlsxPaths:
        ...
        if NEW_RAWDATA_STEP0023_STATUTORY_WELFARE_FILE_PATTERN.match(objResolvedInputPath.name) is not None:
        ...
        if NEW_RAWDATA_STEP0026_STATUTORY_WELFARE_FILE_PATTERN.match(objResolvedInputPath.name) is not None:
            objNewRawdataStep0026StatutoryWelfarePaths.append(objResolvedInputPath)
        if NEW_RAWDATA_STEP0027_STATUTORY_WELFARE_FILE_PATTERN.match(objResolvedInputPath.name) is not None:
            objNewRawdataStep0027StatutoryWelfarePaths.append(objResolvedInputPath)
        if NEW_RAWDATA_STEP0028_STATUTORY_WELFARE_FILE_PATTERN.match(objResolvedInputPath.name) is not None:
            objNewRawdataStep0028StatutoryWelfarePaths.append(objResolvedInputPath)
        if NEW_RAWDATA_STEP0029_STATUTORY_WELFARE_FILE_PATTERN.match(objResolvedInputPath.name) is not None:
            objNewRawdataStep0029StatutoryWelfarePaths.append(objResolvedInputPath)
    ...
    if objNewRawdataStep0005Paths:
        for objNewRawdataStep0005Path in objNewRawdataStep0005Paths:
            ...
            try:
                ...
                objHandledInputPaths.add(objNewRawdataStep0005Path.resolve())
                objHandledInputPaths.add(objNewRawdataStep0006Path.resolve())
                append_make_rawdata_step_result_log(
                    objInputPath=objNewRawdataStep0005Path,
                    pszRunId=pszRunId,
                    pszStepName="step0006_from_step0005",
                    pszResult="success",
                    objOutputPath=objNewRawdataStep0006Path,
                )
            except Exception as objException:
                print(
    ...
    if objNewRawdataStep0023StatutoryWelfarePaths:
        for objNewRawdataStep0023StatutoryWelfarePath in objNewRawdataStep0023StatutoryWelfarePaths:
            if objNewRawdataStep0023StatutoryWelfarePath.resolve() in objHandledInputPaths:
                continue
            try:
                process_new_rawdata_step0024_statutory_welfare_from_step0023_statutory_welfare_with_org_table(
                    objNewRawdataStep0023StatutoryWelfarePath
                )
                objHandledInputPaths.add(objNewRawdataStep0023StatutoryWelfarePath.resolve())
            except Exception as objException:
                print(
                    "Error: failed to process step0024 from step0023 statutory welfare: {0}. Detail = {1}".format(
                        objNewRawdataStep0023StatutoryWelfarePath,
                        objException,
                    )
                )
    ...
    if objNewRawdataStep0024StatutoryWelfarePaths:
        for objNewRawdataStep0024StatutoryWelfarePath in objNewRawdataStep0024StatutoryWelfarePaths:
            if objNewRawdataStep0024StatutoryWelfarePath.resolve() in objHandledInputPaths:
                continue
            try:
                process_new_rawdata_step0025_statutory_welfare_from_step0024_statutory_welfare_with_org_table(
                    objNewRawdataStep0024StatutoryWelfarePath
                )
                objHandledInputPaths.add(objNewRawdataStep0024StatutoryWelfarePath.resolve())
            except Exception as objException:
                print(
    ...
    if objNewRawdataStep0026StatutoryWelfarePaths:
        for objNewRawdataStep0026StatutoryWelfarePath in objNewRawdataStep0026StatutoryWelfarePaths:
            if objNewRawdataStep0026StatutoryWelfarePath.resolve() in objHandledInputPaths:
                continue
            try:
                process_new_rawdata_step0027_statutory_welfare_from_step0026_statutory_welfare(
                    objNewRawdataStep0026StatutoryWelfarePath
                )
                objHandledInputPaths.add(objNewRawdataStep0026StatutoryWelfarePath.resolve())
            except Exception as objException:
                print(
                    "Error: failed to process step0027 from step0026 statutory welfare: {0}. Detail = {1}".format(
                        objNewRawdataStep0026StatutoryWelfarePath,
                        objException,
                    )
                )
    ...
    if objNewRawdataStep0027StatutoryWelfarePaths:
        for objNewRawdataStep0027StatutoryWelfarePath in objNewRawdataStep0027StatutoryWelfarePaths:
            if objNewRawdataStep0027StatutoryWelfarePath.resolve() in objHandledInputPaths:
                continue
            try:
                process_new_rawdata_step0028_statutory_welfare_from_step0027_statutory_welfare(
                    objNewRawdataStep0027StatutoryWelfarePath
                )
                objHandledInputPaths.add(objNewRawdataStep0027StatutoryWelfarePath.resolve())
            except Exception as objException:
                print(
                    "Error: failed to process step0028 from step0027 statutory welfare: {0}. Detail = {1}".format(
                        objNewRawdataStep0027StatutoryWelfarePath,
                        objException,
                    )
                )
    ...
    if objNewRawdataStep0028StatutoryWelfarePaths:
        for objNewRawdataStep0028StatutoryWelfarePath in objNewRawdataStep0028StatutoryWelfarePaths:
            if objNewRawdataStep0028StatutoryWelfarePath.resolve() in objHandledInputPaths:
                continue
            try:
                process_new_rawdata_step0029_statutory_welfare_from_step0028_statutory_welfare(
                    objNewRawdataStep0028StatutoryWelfarePath
                )
                objHandledInputPaths.add(objNewRawdataStep0028StatutoryWelfarePath.resolve())
            except Exception as objException:
                print(
                    "Error: failed to process step0029 from step0028 statutory welfare: {0}. Detail = {1}".format(
                        objNewRawdataStep0028StatutoryWelfarePath,
                        objException,
                    )
                )
    ...
    if objNewRawdataStep0029StatutoryWelfarePaths:
        for objNewRawdataStep0029StatutoryWelfarePath in objNewRawdataStep0029StatutoryWelfarePaths:
            if objNewRawdataStep0029StatutoryWelfarePath.resolve() in objHandledInputPaths:
                continue
            try:
                process_new_rawdata_step0030_statutory_welfare_from_step0029_statutory_welfare(
                    objNewRawdataStep0029StatutoryWelfarePath
                )
                objHandledInputPaths.add(objNewRawdataStep0029StatutoryWelfarePath.resolve())
            except Exception as objException:
                print(
                    "Error: failed to process step0030 from step0029 statutory welfare: {0}. Detail = {1}".format(
                        objNewRawdataStep0029StatutoryWelfarePath,
                        objException,
                    )
                )
